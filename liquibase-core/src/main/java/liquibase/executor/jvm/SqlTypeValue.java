 ...
 * <code>setTypeValue</code> which can throw SQLExceptions that will be caught and translated by the calling code. This
 * callback method has access to the underlying Connection via the given PreparedStatement object, if that should be
...
interface SqlTypeValue {
    ...
    /**
     * Constant that indicates an unknown (or unspecified) SQL type. Passed into <code>setTypeValue</code> if the
     * original operation method does not specify a SQL type.
     * 
     * @see java.sql.Types
     */
     ...
     * @param paramIndex
     *            the index of the parameter for which we need to set the value
     * @param sqlType
     *            SQL type of the parameter we are setting
     * @param typeName
     *            the type name of the parameter (optional)
     * @throws java.sql.SQLException
     *             if a SQLException is encountered setting parameter values (that is, there's no need to catch
     *             SQLException)
     * @see java.sql.Types
     * @see java.sql.PreparedStatement#setObject
     */
