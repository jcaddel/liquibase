...
public class StringChangeLogSerializerTest {
	...
	// ChangeSet changeSet = new ChangeSet("1", "ted", true, false, "com/example/test.xml", "c:/com/exmple/test",
	// "context1, context2", "mysql, oracle");
	// AddColumnChange change = new AddColumnChange();
	...
	public void tryAllChanges() throws Exception {
		...
		Map<String, SortedSet<Class<? extends Change>>> registry = factory.getRegistry();
		for (SortedSet<Class<? extends Change>> changeClassSet : registry.values()) {
			Iterator<Class<? extends Change>> itr = changeClassSet.iterator();
			Class<? extends Change> clazz = itr.next();
			Constructor<? extends Change> constructor = clazz.getConstructor();
			Change change = constructor.newInstance();
			setFields(change);
	...
	private void setFields(Object object) throws Exception {
		...
		for (Field field : clazz.getDeclaredFields()) {
			if (field.getAnnotation(ChangeProperty.class) != null
					&& !field.getAnnotation(ChangeProperty.class).includeInSerialization()) {
				continue;
			}
				...
				if (genericType instanceof ParameterizedType) {
					...
					if (genericsLength == 1) {
						...
						if (typeToCreate.equals(ColumnConfig.class)) {
							...
							throw new RuntimeException("Unknown generic type for " + clazz.getName() + "."
									+ field.getName() + ": " + typeToCreate.getName());
						}
						field.set(object, collection);
					} else {
						throw new RuntimeException("Found " + genericsLength + " generics for " + clazz.getName() + "."
								+ field.getName());
					}
				} else {
